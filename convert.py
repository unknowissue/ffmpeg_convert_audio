...
with open('time_name.txt', 'r') as f:
    ...
    for line in data:
        ...
        if i > 1:
            ...
            if diff_time == 0:
                ##时间没有差值：已经读取到最后一行
                os.system('ffmpeg -ss %s  -i %s  -c:v libx264 -c:a aac -strict experimental -b:a 320k -avoid_negative_ts 1  %s.aac' % (s_time,file_name,mp3_filename))
            else:
                ##时间没有差值：存在最后一行
                os.system('ffmpeg -ss %s  -i %s -t %s -c:v libx264 -c:a aac -strict experimental -b:a 320k -avoid_negative_ts 1  %s.aac' % (s_time,file_name,diff_time,mp3_filename))
